...
class main_listener implements EventSubscriberInterface
{
    ...
    public function add_page_header_link($event)
    {
		...
		if ($this->request->variable('f', 0) > 0)
		{
			...
			{
				$this->template->assign_var('U_ARCHIVE_PAGE', $this->helper->route('archive_base_controller', array('f' => $this->request->variable('f', 0), 't' => $this->request->variable('t', 0))));
			}
			else
			{
				$this->template->assign_var('U_ARCHIVE_PAGE', $this->helper->route('archive_base_controller', array('f' => $this->request->variable('f', 0))));
			}
		}
		else
		{
